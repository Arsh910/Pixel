...
@login_required
def delete1(requests,pk):
        p1=Post_first.objects.filter(id=pk)
        p1.delete()
        return HttpResponseRedirect(f'{pk}/profile')

@login_required
def delete2(requests,pk):
        p2=Post_second.objects.filter(id=pk)
        p2.delete()
        return HttpResponseRedirect(f'{pk}/profile')

@login_required
def delete3(requests,pk):
        p3=Post_third.objects.filter(id=pk)
        p3.delete()
        return HttpResponseRedirect(f'{pk}/profile')

# @login_required
# def delete4(requests,pk,user):
#         p3=Post_fourth.objects.filter(id=pk,auname=user)
#         p3.delete()
